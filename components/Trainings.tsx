...
export const TrainingsComponent: React.FC<TrainingsProps> = ({ ipos, trainings, setTrainings, onSelectIpo, uacsCodes }) => {
    ...
    const processedTrainings = useMemo(() => {
        ...
        if (!canViewAll && currentUser) {
            filtered = filtered.filter(t => t.operatingUnit === currentUser.operatingUnit);
        }
        ...
        return filtered;
    }, [trainings, searchTerm, componentFilter, sortConfig, tableRegionFilter, ipos, currentUser, canViewAll]);
    ...
    // ... handleDownloadTemplate (omitted for brevity, similar to Subprojects) ...
    const handleDownloadTemplate = () => {
        const exampleData = [{
            component: 'Social Preparation', name: 'Basic Leadership Training', date: '2024-03-15', description: 'Training on leadership skills for IPO officers.',
            location: 'Brgy. San Isidro, Tanay, Rizal', facilitator: 'John Doe', participatingIpos: 'San Isidro Farmers Association',
            participantsMale: 15, participantsFemale: 20, expenses: '[{"objectType":"MOOE","expenseParticular":"Training Expenses","uacsCode":"50202010-01","obligationMonth":"2024-03-01","disbursementMonth":"2024-03-20","amount":45000}]',
            ...
            <div className="bg-white dark:bg-gray-800 p-6 rounded-xl shadow-lg">
                 <div className="mb-4 flex flex-col md:flex-row gap-4">
                    <div className="flex flex-wrap gap-x-4 gap-y-2 items-center">
                        <input type="text" placeholder="Search trainings..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} className={`w-full md:w-auto ${commonInputClasses} mt-0`} />
                        ...
                        <div className="flex items-center gap-2">
                           <label htmlFor="componentFilter" className="text-sm font-medium text-gray-700 dark:text-gray-300">Component:</label>
                            <select id="componentFilter" value={componentFilter} onChange={(e) => setComponentFilter(e.target.value as any)} className={`${commonInputClasses} mt-0`}>
                                <option value="All">All Components</option>{trainingComponents.map(c => <option key={c} value={c}>{c}</option>)}
                            </select>
                        </div>
                    </div>
                            ...
                            {paginatedTrainings.map((training) => {
                                ...
                                <React.Fragment key={training.id}>
                                    <tr onClick={() => handleToggleRow(training.id)} className="cursor-pointer hover:bg-gray-50 dark:hover:bg-gray-700/50">
                                        <td className="px-4 py-4 text-gray-400"><svg xmlns="http://www.w3.org/2000/svg" className={`h-5 w-5 transition-transform duration-200 ${expandedRowId === training.id ? 'transform rotate-90' : ''}`} fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 5l7 7-7 7" /></svg></td>
                                        ...
                                        <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500 dark:text-gray-300">{training.operatingUnit}</td>
                                                    ...
                                                    <div className="space-y-4">
                                                        <div>
                                                            ...
                                                            {training.facilitator && <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">Facilitator: {training.facilitator}</p>}
                                                             <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">Encoded by: {training.encodedBy}</p>
                                                        </div>
                                                        ...
                                                        <h4 className="font-semibold text-md mb-2 text-gray-700 dark:text-gray-200">Budget & Funding</h4>
                                                         {training.expenses.length > 0 ? (
                                                             <ul className="space-y-1">
                                                                ...
                                                                <li className="flex justify-between items-center p-1 border-t border-gray-300 dark:border-gray-600 mt-1 pt-1 font-bold"><span>Total</span><span>{formatCurrency(totalTrainingBudget)}</span></li>
                                                            </ul>
                 ...
                 <div className="py-4 flex items-center justify-between">
                    <div className="flex items-center gap-2 text-sm">
                     ...
                     <div className="flex items-center gap-4 text-sm">
                        <span className="text-gray-700 dark:text-gray-300">Showing {Math.min((currentPage - 1) * itemsPerPage + 1, processedTrainings.length)} to {Math.min(currentPage * itemsPerPage, processedTrainings.length)} of {processedTrainings.length} entries</span>
                        <div className="flex items-center gap-2">
    ...
    // ... renderFormView (omitted for brevity, largely same logic as Subprojects, ensure canEdit check is consistent if you want to hide form altogether for non-editors, but 'view' state handles that) ...
            ...
            <form onSubmit={handleSubmit}>
                        ...
                        <div className="space-y-6">
                             <fieldset className="border border-gray-300 dark:border-gray-600 p-4 rounded-md">
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                     <div><label htmlFor="component" className="block text-sm font-medium">Component</label><select name="component" id="component" value={formData.component} onChange={handleInputChange} required className={commonInputClasses}>{trainingComponents.map(c => <option key={c} value={c}>{c}</option>)}</select></div>
                                     <div><label htmlFor="name" className="block text-sm font-medium">Training Name</label><input type="text" name="name" id="name" value={formData.name} onChange={handleInputChange} required className={commonInputClasses} /></div>
                ...
                <div className="flex justify-between items-center pt-4 mt-4 border-t border-gray-200 dark:border-gray-700">
                    ...
                    <div className="flex gap-4">
                        ...
                        <button type="submit" className="inline-flex justify-center py-2 px-4 border border-transparent shadow-sm text-sm font-medium rounded-md text-white bg-accent hover:brightness-95 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-accent">{editingTraining ? 'Update Training' : 'Add Training'}</button>
                    </div>
                </div>
            </form>
        </div>
