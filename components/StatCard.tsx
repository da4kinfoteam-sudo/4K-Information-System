...
const StatCard: React.FC<StatCardProps> = ({ title, value, icon, color, onClick }) => {
    return (
        <div 
            onClick={onClick}
            className={`bg-white dark:bg-gray-800 p-4 lg:p-5 rounded-xl shadow-lg flex items-center space-x-4 hover:shadow-xl hover:scale-105 transform transition-all duration-300 ease-in-out min-w-0 ${onClick ? 'cursor-pointer ring-2 ring-transparent hover:ring-accent/20' : ''}`}
        >
