...
const BAR1Table: React.FC<{ data: { [key: string]: any } }> = ({ data }) => {
    const [expandedRows, setExpandedRows] = useState(new Set<string>());
    const toggleRow = (key: string) => {
        setExpandedRows(prev => {
            const newSet = new Set(prev);
    ...
    const grandTotals = Object.values(data).flatMap((component: any) => {
...
const Reports: React.FC<ReportsProps> = ({ ipos, subprojects, trainings, otherActivities, officeReqs, staffingReqs, otherProgramExpenses, uacsCodes }) => {
    ...
    const bar1Data = useMemo(() => {
        ...
        processPm(filteredData.staffingReqs, 'Staff Requirements', true);
        processPm(filteredData.officeReqs, 'Office Requirements');
        processPm(filteredData.otherProgramExpenses, 'Office Requirements');
